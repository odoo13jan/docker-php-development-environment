...
(function($){
   ...
   table=$('#groups_table').DataTable( {
      ...
      "columns": [
         ...
         {data: "doctor.name", orderable: false, sortable: false, defaultContent: '' },
   ...
   $(document).ready(function() {
        $(document).on('submit', 'form', function (e) {
            ...
            // Check if payment method is selected in each row
            $('.payment_method_id').each(function() {
                if ($(this).val() === "") {
                    valid = false;
                    $(this).siblings('.error-message').show(); // Show the error message
                } else {
                    $(this).siblings('.error-message').hide(); // Hide the error message if valid
                }
            });
   ...
   $(document).on('select2:select','#code', function (e) {
    ...
    // Get patient by phone select2
    $('#phone_select').select2({
        width: "100%",
        placeholder: trans("Patient Phone"),
        ajax: {
            beforeSend: function() {
                $('.preloader').show();
                $('.loader').show();
            },
            url: ajax_url('get_patient_by_phone'),
            processResults: function(data) {
                return {
                    results: $.map(data, function(item) {
                        return {
    ...
    // Selected phone
    $(document).on('select2:select', '#phone_select', function(e) {
        var el = $(e.target);
        var data = e.params.data;
        $.ajax({
            url: ajax_url('get_patient?id=' + data.id),
            ...
            success: function(patient) {
                // Update code and name dropdowns
      ...
      ajax: {
         ...
         processResults: function (data) {
            return {
                  results: $.map(data, function (item) {
                     return {
                        text: item.name+' ( '+item.category.name+' )'+' ( '+item.price+' )'+' ( '+item.separated+' )'     ,
                        id: item.id,
                     }
                  })
            };
