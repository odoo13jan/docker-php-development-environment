...
class ReportsController extends Controller
{
    ...
    public function doctor(Request $request)
    {
        if($request->has('date'))
        {
            ...
            //make accounting
            $total=0; $paid=0; $due=0; $labtotal=0; $labdue=0;$labdiscount=0;$labsubtotal=0;$lablabpaid=0;$labdoctor_commission=0;
            
            $labsubtotal+=$groups->sum('subtotal');
            $labdiscount+=$groups->sum('discount');
            $labtotal+=$groups->sum('total');
            $lablabpaid+=$groups->sum('paid');
            $labdue+=$groups->sum('due');
            $labdoctor_commission+=$groups->sum('doctor_commission');
