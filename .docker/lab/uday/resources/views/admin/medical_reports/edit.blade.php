              ...
              <table class="table table-striped table-bordered">
                <thead>
                  ...
                  <!--  <th width="200px" class="text-center">{{__('Result')}}</th>-->
                    <!--<th width="200px" class="text-center">{{__('Text')}}</th>-->
                  ...
                  <tr>
                    ...
                    <th  class="text-center">{{__('Reference Range')}}</th>
                      ...
                      @if($result['component']['title'])
                        <tr>
                          ...
                          @if(!$result['component']['result_temp'])
                            <td colspan="3" class="component_title test_name"></td>
                          @elseif(!$result['result_temp'])
                            <td colspan="3" class="component_title test_name">
                                <textarea name="result[{{$result['id']}}][result_temp]" class="form-control" required>{{$result['component']['result_temp']}}</textarea>
                            </td>
                          @elseif($result['result_temp'])
                            <td colspan="3" class="component_title test_name">
                                <textarea name="result[{{$result['id']}}][result_temp]" class="form-control" required>{{$result['result_temp']}}</textarea>
                            </td>
                          @endif
                        </tr>
                      @else
                        <tr>
                          ...
                          <!--  @if(isset($result['component']['reference_ranges']) && count($result['component']['reference_ranges']))-->
                          <!--    @foreach($result['component']['reference_ranges'] as $reference_range)-->
                          <!--      ({{ __($reference_range['gender']) }} ) {{ $reference_range['normal_from'] }} - {{ $reference_range['normal_to'] }} {{ $result['component']['unit'] }}-->
                          ...
                          <td class="text-center">
                            @if(isset($result['component']['reference_ranges']) && count($result['component']['reference_ranges']))
                                @foreach($result['component']['reference_ranges'] as $reference_range)
                                    <div>({{ __($reference_range['gender']) }} ) {{ $reference_range['normal_from'] }} - {{ $reference_range['normal_to'] }} {{ $result['component']['unit'] }}</div>
                                @endforeach
                            @else
                            <!---->
                          ...
                          <td>
                            @if($result['component']['type']=='text')
                              <input 
                                ...
                                class="form-control test_result" 
                                value="{{$result['result']}}" 
                                ...
                                @endif
                                @if(in_array($result['component']['name'], [
                                    'TOTAL BILIRUBIN', 'DIRECT BILIRUBIN', 'IN-DIRECT BILIRUBIN',
                                    'SGOT', 'SGPT', 'ALKALINE PHOSPHATE', 'TOTAL PROTEIN', 'ALBUMIN', 'GLOBULIN', 'Albumin / Globulin Ratio ( A/G )',
                                    
                                    'Total Cholesterol', 'H D L Cholesterol', 'Triglycerides', 'L D L Cholesterol DIRECT', 'VLDL Cholesterol',
                                    'TCHOL/HDL Ratio', 'TCHOL/TGL Ratio', 'LDL/HDL Ratio',
                                    ...
                                    'Neutrophils', 'Lymphocytes', 'Eosinophils', 'Monophils', 'Basophils'
                                ]))
                                  data-calc-field="{{$result['component']['name']}}"
                                  data-calc-test="{{$test['test']['name']}}"
                            ...
                            @if($result['component']['status'])
                              <select name="result[{{$result['id']}}][status]" class="form-control select_result status">
                                ...
                                @if(!empty($result['status']) && !in_array($result['status'], ['High', 'Normal', 'Low', 'Critical high', 'Critical low']))
                                  <option value="{{ $result['status'] }}" disabled selected>{{ $result['status'] }}</option>
                                @endif
                              </select>
                            @endif
                          </td>
...
<script src="{{url('js/admin/test_calculations.js')}}"></script>

<script>
    // Capture Enter key and move to the next input field
    document.addEventListener('DOMContentLoaded', function () {
        // Get all input fields in the form
        let inputs = document.querySelectorAll('input.test_result');
