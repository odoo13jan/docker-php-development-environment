@extends('layouts.pdf')
@section('title')
    {{__('Doctor report')}}-{{$data['doctor']['id']}}-{{date('Y-m-d')}}
@endsection
@section('content')
<style>
    .title{
        font-size: 20px;
        background-color: #dddddd;
        border: 1px solid black!important;
        margin-bottom: 10px;
    }
    .table{
        margin-top: 20px;
    }
    .accounting_header{
        border: 1px solid black;
        background-color: #F0F0F0;
        font-family: 'amiri';
        
    }
    th{
        font-family: 'amiri';
        
    }
    ul{
         font-size: 10px;
    }
</style>
    
    
    <div class="accounting_header">
        <h4 align="center">
            <img  src="data:image/png;base64," width="15px" alt=""> 
            {{__('Doctor')}} : {{$data['doctor']['name']}}<br>
            <!--{{__('Due Date')}} : {{date('Y-m-d')}}<br>-->
            Branch Name : {{session('branch_name')}}<br>
            {{__('From Date')}} {{date('Y-m-d',strtotime($data['from']))}} {{__('To Date')}} {{date('d-m-Y',strtotime($data['to']))}}
        </h4>
    </div>
    <!-- Report Details -->
    <table class="table table-bordered">
        <thead>
        <tr class="title">
            <th colspan="9">
                {{__('Group Tests')}}
            </th>
        </tr>
        <tr>
            <th>{{__('Date')}}</th>
            <th>{{__('Patient Name')}}</th>
            <th>{{__('Tests')}}</th>
            <th>{{__('Subtotal')}}</th>
            <th>{{__('Discount')}}</th>
            <th>{{__('Total')}}</th>
            <th>{{__('Paid')}}</th>
            <th>{{__('Due')}}</th>
            <th>{{__('Ref.')}}</th>
        </tr>
        </thead>
        <tbody>
        @if(count($data['groups'])==0)
        <tr>
            <td colspan="9" align="center">
                {{__('No data available')}}
            </td>
        </tr>
        @endif
        
        @foreach($data['groups'] as $group)
        <tr>
            <td align="left">
                {{date('d-m-Y H:i',strtotime($group['sample_collection_date']))}}
            </td>
            <td align="center">
            @if(isset($group['patient']))
                {{$group['patient']['name']}}
            @endif
            </td>
            <td>
                <ul class="p-1"  >
                    @foreach($group['tests'] as $test)
                      <li>{{$test['test']['name']}}</li>
                    @endforeach
                    @foreach($group['cultures'] as $culture)
                      <li>{{$culture['culture']['name']}}</li>
                    @endforeach
                </ul>
                @foreach($group['packages'] as $package)
                    <b>{{$package['package']['name']}}</b>
                    <ul>
                        @foreach($package['tests'] as $test)
                            <li>{{$test['test']['name']}}</li>
                        @endforeach
                        @foreach($package['cultures'] as $culture)
                            <li>{{$culture['culture']['name']}}</li>
                        @endforeach
                    </ul>
                @endforeach
            </td>
            <td align="center">{{($group['subtotal'])}}</td>
            <td align="center">{{($group['discount'])}}</td>
            <td align="center">{{($group['total'])}}</td>
            <td align="center">{{($group['paid'])}}</td>
            <td align="center">{{($group['due'])}}</td>
            <td align="center">{{($group['doctor_commission'])}}</td>
        </tr>
        @endforeach
        <tr>
            <td></td>
            <td></td>
            <td></td>
            <td align="center"> {{($data['labsubtotal'])}}</td>
            <td align="center"> {{($data['labdiscount'])}}</td>
            <td align="center"> {{($data['labtotal'])}}</td>
            <td align="center"> {{($data['lablabpaid'])}}</td>
            <td align="center"> {{($data['labdue'])}}</td>
            <td align="center"> {{($data['labdoctor_commission'])}}</td>
            
        </tr>
        </tbody>
    </table>
    <!-- \Report Details -->

    <!-- Payments Details -->
    <table class="table table-bordered" width="100%">
        <thead>
        <tr class="title">
            <th colspan="4">
                {{__('Payments')}}
            </th>
        </tr>
        <tr>
            <th>#</th>
            <th>{{__('Date')}}</th>
            <th>{{__('Amount')}}</th>
            <th>{{__('Payment method')}}</th>
        </tr>
        </thead>
        <tbody>
        @if(count($data['payments'])==0)
        <tr>
            <td colspan="4" align="center">
                {{__('No data available')}}
            </td>
        </tr>
        @endif
        @foreach($data['payments'] as $payment)
        <tr>
            <td align="center">{{$payment['id']}}</td>
            <td align="center">{{date('d-m-Y',strtotime($payment['date']))}}</td>
            <td align="center">{{formated_price($payment['amount'])}}</td>
            <td align="center">{{$payment['payment_method']['name']}}</td>
        </tr>
        @endforeach
        </tbody>
    </table>
    <!-- \Payments Details -->

    <!--  Report Summary  -->    
    <table class="table table-bordered" width="100%">
        <thead>
            <tr class="title">
                <th colspan="12">
                    {{__('Accounting Report Summary')}}
                </th>
            </tr>
        </thead>
        <tbody>
        <tr>
            <th  align="center">{{__('Ref. Total')}}:</th>
            <td>{{($data['total'])}}</td>
        <!--</tr>-->
        <!--<tr>-->
            <th align="center">{{__('Ref. Paid')}}:</th>
            <td>{{($data['paid'])}}</td>
        <!--</tr>-->
        <!--<tr>-->
            <th align="center">{{__('Ref. Due')}}:</th>
            <td>{{($data['due'])}}</td>
            
            <th align="center">{{__('Lab Total')}}:</th>
            <td>{{($data['labtotal'])}}</td>
        <!--</tr>-->
        <!--<tr>-->
            <th align="center">{{__('Lab Paid')}}:</th>
            <td>{{($data['lablabpaid'])}}</td>
        <!--</tr>-->
        <!--<tr>-->
            <th align="center">{{__('Lab Due')}}:</th>
            <td>{{($data['labdue'])}}</td>
            
        </tr>
        
        </tbody>
    </table>
    <!-- \ Report Summary-->
@endsection